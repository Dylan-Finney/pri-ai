...
export default function SetupAI(props){
    const [aIName, setAIName, , , selectedAvatar, setSelectedAvatar] = useContext(UserContext);
    return (
        <>
        <Box overflowY={"auto"} scrollBehavior={"smooth"} minHeight={"70vh"} maxHeight={"70vh"}>
                      <Box width={"48px"} height={"48px"} marginBottom={{base: "24px", sm: "0px"}}>
                          <NextImage 
                          src={logoModal}
                          alt="Logo Icon"
                          width={100}
                      ...
                      <FormControl isInvalid={aIName===""  || (props.failedSubmit && aIName === null)}>
                      ...
                      <FormControl isInvalid={selectedAvatar==="" || (props.failedSubmit && selectedAvatar === null)}>
                      <SimpleGrid columns={7} minChildWidth={{base: "70px",sm: '97px'}} spacing='10px' marginTop={"10px"} marginRight={{base: "0px", sm: "57px"}}>
                        {
                          avatars.map((avatar,i)=>{
                            if (avatar === selectedAvatar) {
                              return (
                                <Box maxWidth={{base: "70px", sm: "90px"}} key={i} src={`/assets/avatar/${avatar}`} alt={avatar} onClick={()=>{setSelectedAvatar(avatar)}} style={{"padding": "10px", backgroundColor: "#F0FDF9", borderRadius: "50px", border: "2px solid #0E9384"}}>
                                  <NextImage 
                              ...
                              return (
                                <Box key={i} maxWidth={{base: "70px", sm: "90px"}}  onClick={()=>{setSelectedAvatar(avatar)}} style={{"padding": "10px", borderRadius: "50px",border: "2px solid #EAECF0"}}>
                                  <NextImage 
                                  src={`/assets/avatar/${avatar}`}
                                  alt={`${avatar}`}
                                  width={100}
